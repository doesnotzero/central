...
const APP_SUBTITLE = "DOES NOT ZERO";
const DNZ_LOGO_URL = "/dnz-assets/dnz-logo-nav.webp";
...
const PLANS = [
  ...
  { id:"pro", name:"Pro", price:"R$149", period:"/mês", audience:"Freelancer premium", color:"#ff2400", highlight:true,
    promise:"DNZ Central completo para vender, documentar, produzir e receber melhor.",
    features:["Tudo do Solo","DNZ Score","Studio de documentos PDF","Histórico de documentos","Backup criptografado","Relatórios premium"],
  ...
  { id:"white_label", name:"White Label", price:"Sob consulta", period:"", audience:"Mentorias, escolas e comunidades", color:"#3b82f6", highlight:false,
    promise:"DNZ Central com sua marca para vender metodologia, operação e documentos profissionais.",
    ...
    limits:"Ideal para quem quer vender DNZ Central como método ou sistema próprio.",
...
const DEFAULT_BUSINESS = {
  ...
  companyName:"Does Not Zero",
  logoUrl:DNZ_LOGO_URL,
  primaryColor:"#ff2400",
...
const normalizeBusiness = b => {
  ...
  if(!next.logoUrl)next.logoUrl=DNZ_LOGO_URL;
  return next;
};
...
const LEVELS = [
  ...
  { min: 2000, name: "Imparável",    color: "#ff2400" },
...
const C = {
  ...
  orange:"#ff2400", orangeD:"#cc1d00", text:"#e8e8e8", muted:"#666", faint:"#2a2a2a"
};
const CLIENT_PIPELINE = [
  {key:"lead",label:"Lead",color:"#3b82f6",hint:"Contato novo, ainda sem briefing fechado"},
  {key:"briefing",label:"Briefing",color:"#06b6d4",hint:"Entender escopo, prazo, referência e verba"},
  {key:"proposta_enviada",label:"Proposta enviada",color:"#eab308",hint:"PDF enviado, aguardando decisão"},
  {key:"em_producao",label:"Em produção",color:"#8b5cf6",hint:"Projeto aprovado, produção em andamento"},
  {key:"entregue",label:"Entregue",color:"#10b981",hint:"Material entregue ou em aceite final"},
  {key:"pago",label:"Pago",color:"#22c55e",hint:"Contrato recebido e encerrado"},
];
const STATUS_COLORS = Object.fromEntries(CLIENT_PIPELINE.map(s=>[s.key,s.color]));
const LEGACY_CLIENT_STATUS = { prospecto:"lead", ativo:"em_producao", pausado:"proposta_enviada", concluido:"entregue" };
const normalizeClientStatus = c => c?.status==="concluido"&&c?.payment==="pago" ? "pago" : (LEGACY_CLIENT_STATUS[c?.status] || c?.status || "lead");
const clientStatusLabel = status => (CLIENT_PIPELINE.find(s=>s.key===status)||CLIENT_PIPELINE[0]).label;
...
const getPlanMeta = planId => planId==="admin" ? {id:"admin",name:"Admin",color:C.orange,price:"Acesso total",features:["Todos os recursos"]} : (PLANS.find(p=>p.id===planId)||{id:"trial",name:"Restrito",color:"#eab308",price:"Liberação interna",features:["Acesso temporário"]});
const hasPlanAccess = (state,required="solo",isAdmin=false) => (PLAN_ORDER[effectivePlanId(state,isAdmin)]||0) >= (PLAN_ORDER[required]||0);
const salesMessageForPlan = plan => `Olá, preciso liberar a camada ${plan.name} no ${APP_NAME}. Meu foco é: ${(plan.deliverables||plan.features||[]).slice(0,3).join(", ")}.`;
...
const LogoMark = ({business,size=58,textColor})=>{
  ...
  const src=business?.logoUrl||DNZ_LOGO_URL;
  useEffect(()=>setFailed(false),[src]);
  if(src&&!failed)return <img src={src} alt={`Logo ${business?.brandName||APP_NAME}`} onError={()=>setFailed(true)} style={{width:"100%",height:"100%",objectFit:"contain",display:"block"}}/>;
...
const Modal = ({open,onClose,title,children,wide})=>{
  ...
  return (
    <div className="modal-shell" role="presentation">
      ...
      <div ref={dialogRef} tabIndex={-1} className={`scale-in modal-panel ${wide?"wide":""}`} role="dialog" aria-modal="true" aria-label={title||"Janela"}>
...
const PlanBadge = ({state,isAdmin=false})=>{
  ...
  return <span className="plan-badge" style={{color:plan.color,borderColor:`${plan.color}44`,background:`${plan.color}12`}}>
    {plan.name} · {isAdmin?"acesso total":sub.status==="active"?"ativo":sub.status==="expired"?"expirado":"restrito"}
  </span>;
};
const PlanGate = ({state,isAdmin=false,required="pro",title="Recurso premium",text="Esse recurso faz parte da camada paga da DNZ Central.",setTab})=>{
  ...
  return (
    <div className="plan-gate">
      ...
      <Btn onClick={()=>setTab?.("business")} size="sm">Abrir operação</Btn>
    </div>
  );
};
...
const PlanDeliverables = ({plan,recommended,active,onBuy,onActivate,isAdmin})=>(
  <div className={`plan-card ${plan.highlight?"featured":""}`} style={{"--accent":plan.color}}>
    ...
    <div style={{display:"grid",gap:8}}>
      <a href={onBuy} target="_blank" rel="noopener noreferrer" className="elite-primary" style={{background:`linear-gradient(135deg,${plan.color},${C.orangeD})`,color:"#fff",minHeight:40}}>Solicitar liberação</a>
      {isAdmin&&<button onClick={onActivate} className="elite-secondary" style={{minHeight:38}}>Liberar camada</button>}
      {active&&<div style={{fontSize:10,color:plan.color,fontWeight:900,textAlign:"center"}}>Camada ativa neste ambiente</div>}
    </div>
  </div>
);
...
const AccessWall = ({onLogin})=>(
  <div className="access-wall">
    <div className="access-wall-card scale-in">
      <div className="elite-kicker">ACESSO INTERNO</div>
      <h1 style={{fontSize:"clamp(30px,5vw,52px)",lineHeight:1,color:"#fff",fontFamily:"'Syne',sans-serif",margin:"10px 0 12px"}}>Workspace privado da DNZ Films.</h1>
      <p style={{fontSize:15,color:"#cfcfcf",lineHeight:1.65,maxWidth:640,margin:"0 0 20px"}}>Este ambiente concentra clientes, propostas, produção, review, entrega e financeiro. A entrada é liberada apenas para a conta administrativa autorizada.</p>
      <div className="access-steps">
        {[
          ["1","Produção","Projetos, prazos e entregáveis."],
          ["2","Review","Aprovação de vídeos com cliente."],
          ["3","Financeiro","Recebimentos e histórico."],
...
const NotificationsBanner = ({state,setTab})=>{
  ...
  if(habitsMissing.length>0) msgs.push({id:"habits_missing",icon:"🔥",color:"#ff2400",text:`${habitsMissing.length} hábito${habitsMissing.length>1?"s":""} pendente${habitsMissing.length>1?"s":""} hoje`,action:()=>setTab("habits")});
...
const ContextAlert = ({tab,state,setTab,notify})=>{
  ...
  const followUps=clients.filter(c=>c.followUpDate&&dayDiff(c.followUpDate)<=0&&normalizeClientStatus(c)!=="pago").length;
...
const TabProposta = ({state,dispatch})=>{
  ...
  const [objective,setObjective]=useState("Transformar a ideia em uma peça audiovisual com narrativa, ritmo e acabamento prontos para publicação.");
  const [packageName,setPackageName]=useState("MOTION");
  const [deliverables,setDeliverables]=useState("Video principal editado\nCortes para redes sociais\nColor grading cinematografico\nTrilha / tratamento sonoro\nEntrega em formatos finais");
  const [timeline,setTimeline]=useState("Briefing e alinhamento criativo\nProducao / captacao\nEdicao e primeira versao\nRevisao final\nEntrega dos arquivos");
  ...
  const escPdf=v=>String(v??"").replace(/[&<>"']/g,m=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[m]));
  const lines=v=>String(v||"").split(/\n+/).map(x=>x.trim()).filter(Boolean);

  const PACKAGE_PRESETS={
    LOOP:{
      objective:"Manter a marca presente com cortes objetivos, ritmo de rede social e entrega recorrente sem complicar produção.",
      deliverables:"3 reels / shorts verticais\nEdição dinâmica com legenda\nTratamento de cor e áudio\nCapa simples para publicação\nEntrega em 9:16",
      timeline:"Briefing rápido\nRecebimento ou captação do material\nEdição dos cortes\nAjustes finais\nEntrega para postagem",
      services:["Edição de vídeo","Reel / Short"]
    },
    MOTION:{
      objective:"Transformar a ideia em uma peça audiovisual com narrativa, ritmo e acabamento prontos para publicação.",
      deliverables:"Vídeo principal editado\nCortes para redes sociais\nColor grading cinematográfico\nTrilha / tratamento sonoro\nEntrega em formatos finais",
      timeline:"Briefing e alinhamento criativo\nProdução / captação\nEdição e primeira versão\nRevisão final\nEntrega dos arquivos",
      services:["Vídeo Institucional","Reel / Short"]
    },
    ZERO:{
      objective:"Desenvolver um projeto audiovisual completo, com direção criativa, produção sob medida e acabamento premium para marca ou campanha.",
      deliverables:"Direção criativa do projeto\nVídeo principal sob medida\nCortes e versões para canais digitais\nTratamento cinematográfico completo\nPacote final de publicação",
      timeline:"Imersão e conceito criativo\nPré-produção e roteiro\nProdução / captação\nPós-produção e revisão\nEntrega final e arquivos master",
      services:["Direção Criativa","Vídeo Institucional","Drone"]
    }
  };
  const applyPackage=name=>{
    const preset=PACKAGE_PRESETS[name];
    if(!preset)return;
    setPackageName(name);
    setObjective(preset.objective);
    setDeliverables(preset.deliverables);
    setTimeline(preset.timeline);
    const picked=SERVICES_CATALOG.filter(s=>preset.services.some(label=>String(s.name).toLowerCase().includes(label.toLowerCase())));
    if(picked.length){
      setSelected(picked.map(s=>({...s,qty:1,customPrice:s.price})));
    }
  };
  ...
  const loadClient=c=>{
    setLoadedClientId(c.id);setLoadedProjectId("");
    setClient({name:c.name||"",company:c.name||"",email:c.email||"",phone:c.phone||"",city:business.proposalCity||"",cnpj:""});
    setProjectTitle(c.service||"Projeto audiovisual DNZ");
  };
  const loadProject=p=>{
    loadClient(p.client);
    ...
    setProjectTitle(p.video.title||preset.title||"Projeto audiovisual DNZ");
    setObjective(p.video.briefing||preset.briefing||"Criar uma entrega audiovisual com direção, ritmo e acabamento alinhados ao posicionamento do projeto.");
    setPackageName((p.video.presetId||p.video.type||preset.id||"motion").toUpperCase().includes("DOC")?"ZERO":"MOTION");
    setDeadline(p.video.deadline||"");
    setDeliverables((p.video.deliverables||presetDeliverables(preset)).map(d=>d.text||d).join("\n"));
    setNotes(`Escopo base importado do projeto ${p.video.title||preset.title}.`);
  };
  const saveProposalToCRM=()=>{
    ...
    const proposal={
      ...
      projectTitle:projectTitle||selected[0]?.name||"",
  ...
  const generatePDF=()=>{
    setGenerating(true);
    const now=new Date();
    const docNum=`DNZ-${now.getFullYear()}-${String(Date.now()).slice(-5)}`;
    const servicesHTML=selected.map(s=>`
      <tr>
        <td><strong>${escPdf(s.name)}</strong><span>${escPdf(s.desc||"Entrega audiovisual DNZ")}</span></td>
        <td>${s.qty}x</td>
        <td>${fmtCurrency(s.price)}</td>
        <td>${fmtCurrency(s.price*s.qty)}</td>
      </tr>
    `).join("");
    const logoSrc=business.logoUrl||DNZ_LOGO_URL;
    const html=`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Proposta — ${client.name||"Cliente"}</title>
    <link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&family=DM+Sans:wght@400;600;700;900&family=Space+Mono:wght@400;700&display=swap" rel="stylesheet">
    <style>
    @page{size:A4;margin:0}
    *{box-sizing:border-box;margin:0;padding:0;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
    html{background:#0b0b0b!important}
    body{background:#0b0b0b!important;color:#f2f2f2;font-family:'DM Sans',Arial,sans-serif;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
    .sheet{width:210mm;min-height:297mm;margin:0 auto;background:#0b0b0b;padding:18mm 22mm 20mm;position:relative;overflow:hidden}
    .sheet::after{content:"";position:absolute;right:-38mm;top:-35mm;width:110mm;height:110mm;border-radius:50%;background:radial-gradient(circle,rgba(255,36,0,.075),transparent 64%);pointer-events:none}
    .top{display:grid;grid-template-columns:1fr 38mm;gap:14mm;align-items:start;padding-bottom:14mm;border-bottom:2px solid #ff2400;position:relative;z-index:1}
    .brand{display:grid;grid-template-columns:14mm 1fr;gap:6mm;align-items:start}.logo{width:14mm;height:14mm;object-fit:contain}.brand-name{font-family:'Bebas Neue',sans-serif;font-size:24pt;line-height:.88;letter-spacing:0;color:#fff}.brand-sub{font-family:'Space Mono',monospace;font-size:8.5pt;line-height:1.7;letter-spacing:3.2pt;text-transform:uppercase;color:#ff2400;font-weight:700;margin-top:3mm}
    .doc{text-align:right;font-family:'Space Mono',monospace;text-transform:uppercase;font-size:8.2pt;letter-spacing:2.2pt;color:#3f3f3f;font-weight:700}.doc b{display:block;color:#ff2400;font-size:14pt;letter-spacing:0;margin:2mm 0}.doc span{letter-spacing:0;color:#333;font-size:9pt;text-transform:none}
    .section{margin-top:12mm;position:relative;z-index:1}.section-title{font-family:'Space Mono',monospace;font-size:9.5pt;letter-spacing:3.2pt;text-transform:uppercase;color:#ff2400;font-weight:900;padding-bottom:4mm;border-bottom:1px solid #202020;margin-bottom:5mm}
    .grid{display:grid;grid-template-columns:1fr 1fr;gap:3mm}.field{background:#151515;border:1px solid #242424;border-radius:3mm;padding:4mm}.field .label{display:block;font-family:'Space Mono',monospace;font-size:8pt;letter-spacing:1.5pt;text-transform:uppercase;color:#4b4b4b;font-weight:900;margin-bottom:2mm}.field b{display:block;color:#f1f1f1;font-size:10pt;font-weight:600}
    table{width:100%;border-collapse:separate;border-spacing:0;background:#151515;border-radius:3mm;overflow:hidden}th{background:#1b1b1b;color:#555;font-family:'Space Mono',monospace;font-size:8pt;letter-spacing:1.6pt;text-transform:uppercase;text-align:left;padding:4mm}td{padding:4mm;border-top:1px solid #242424;color:#eee;font-size:10pt}td strong{display:block;color:#fff;font-weight:900}td span{display:block;color:#606060;margin-top:1.5mm;font-size:9pt}td:nth-child(n+2),th:nth-child(n+2){text-align:right}td:last-child{color:#ff2400;font-weight:900}
    .subtotal{display:flex;justify-content:space-between;align-items:center;background:#111;border:1px solid #2a2a2a;border-radius:3mm;padding:4mm 5mm;margin-top:4mm;color:#777}.subtotal b{color:#ddd}
    .total{display:grid;grid-template-columns:1fr auto;align-items:center;gap:8mm;border:1px solid #ff2400;border-radius:3mm;background:linear-gradient(135deg,rgba(255,36,0,.16),rgba(255,36,0,.035));padding:7mm;margin-top:4mm}.total span{font-family:'Space Mono',monospace;font-size:9pt;letter-spacing:2pt;text-transform:uppercase;color:#ff2400;font-weight:900}.total b{font-size:25pt;color:#fff;font-weight:900}
    .terms{background:#101010;border:1px solid #171717;border-radius:3mm;padding:6mm;color:#3d3d3d;font-size:10pt;line-height:1.8}.terms strong{display:block;font-family:'Space Mono',monospace;letter-spacing:2pt;text-transform:uppercase;color:#444;margin-bottom:3mm}
    .sign{display:grid;grid-template-columns:1fr 1fr;gap:24mm;margin-top:16mm;border-top:1px solid #242424;padding-top:6mm}.sign div{border-top:1px solid #444;text-align:center;padding-top:3mm;color:#3e3e3e;font-size:9pt}
    @media print{html,body{width:210mm;min-height:297mm;background:#0b0b0b!important}.sheet{margin:0;background:#0b0b0b!important;box-shadow:inset 0 0 0 1000mm #0b0b0b}}
    </style></head><body>
    <main class="sheet">
      <header class="top">
        <div class="brand"><img class="logo" src="${escPdf(logoSrc)}" alt="${escPdf(business.brandName||APP_NAME)}"><div><div class="brand-name">DNZ FILMS</div><div class="brand-sub">${escPdf(business.type||"Filmmaker / produtor audiovisual")} · propostas e operacao comercial</div></div></div>
        <div class="doc">Proposta<br/>comercial<b>#${String(Date.now()).slice(-5)}</b><span>${now.toLocaleDateString("pt-BR")}</span></div>
      </header>
      <section class="section">
        <div class="section-title">📋 Dados do cliente</div>
        <div class="grid">
          ${client.name?`<div class="field"><span class="label">Nome / responsavel</span><b>${escPdf(client.name)}</b></div>`:""}
          ${client.company?`<div class="field"><span class="label">Empresa</span><b>${escPdf(client.company)}</b></div>`:""}
          ${client.email?`<div class="field"><span class="label">Email</span><b>${escPdf(client.email)}</b></div>`:""}
          ${client.phone?`<div class="field"><span class="label">WhatsApp</span><b>${escPdf(client.phone)}</b></div>`:""}
          ${client.city?`<div class="field"><span class="label">Cidade</span><b>${escPdf(client.city)}</b></div>`:""}
          <div class="field"><span class="label">Validade da proposta</span><b>${escPdf(validity)} dias</b></div>
        </div>
      </section>
      <section class="section">
        <div class="section-title">🎬 Escopo de servicos</div>
        <table><thead><tr><th>Servico</th><th>Qtd.</th><th>Unitario</th><th>Total</th></tr></thead><tbody>${servicesHTML}</tbody></table>
        <div class="subtotal"><span>Subtotal</span><b>${fmtCurrency(total)}</b></div>
        ${discount>0?`<div class="subtotal"><span>Desconto (${discount}%)</span><b>-${fmtCurrency(discountVal)}</b></div>`:""}
        <div class="total"><span>Valor total do projeto</span><b>${fmtCurrency(finalTotal)}</b></div>
      </section>
      <section class="section">
        <div class="section-title">💳 Condicoes comerciais</div>
        <div class="field"><span class="label">Forma de pagamento</span><b>${escPdf(payTerms)}</b></div>
      </section>
      <section class="section">
        <div class="terms"><strong>Termos & condicoes</strong>Esta proposta tem validade de ${escPdf(validity)} dias a partir da data de emissao. Os prazos de entrega serao definidos apos confirmacao e aprovacao do projeto. Qualquer alteracao de escopo apos o inicio da producao estara sujeita a revisao de valores. O pagamento devera ser realizado conforme acordado. Todos os direitos de imagem e producao audiovisual serao transferidos ao cliente apos quitacao total do contrato.</div>
      </section>
      <div class="sign">
        <div>Equipe Dnz Films<br>${escPdf(business.proposalEmail||"gabrieldpimentel@proton.me")}</div>
        <div>${escPdf(client.name||"Cliente")}<br>Responsavel pela contratacao</div>
      </div>
    </main>
    </body></html>`;
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <Card style={{background:"linear-gradient(135deg,rgba(59,130,246,.1),rgba(0,0,0,0))",borderColor:"rgba(59,130,246,.25)",padding:"20px 22px"}}>
        <div style={{fontSize:11,color:C.orange,fontWeight:900,letterSpacing:".16em",textTransform:"uppercase",marginBottom:8}}>PROPOSTA COMERCIAL DNZ</div>
        <div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:42,lineHeight:.92,color:"#fff",letterSpacing:0,marginBottom:10}}>VENDA O PROJETO,<br/>NAO SO O ORCAMENTO.</div>
        <p style={{margin:0,fontSize:13,color:"#aaa",lineHeight:1.55,maxWidth:760}}>Monte uma proposta A4 com capa, direção criativa, entregáveis, cronograma, investimento e assinatura no padrão Does Not Zero.</p>
      </Card>
      ...
      <Card>
        <SectionTitle>DIREÇÃO DA PROPOSTA</SectionTitle>
        <div style={{display:"grid",gridTemplateColumns:"1.2fr .8fr",gap:"0 16px"}}>
          <Inp label="Título do projeto" value={projectTitle} onChange={setProjectTitle} placeholder="Ex: Filme de lançamento / campanha / cobertura"/>
          <div style={{marginBottom:18}}>
            <div style={{fontSize:11,color:C.muted,marginBottom:7,fontWeight:700,letterSpacing:".08em",textTransform:"uppercase"}}>Pacote DNZ</div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:7}}>
              {["LOOP","MOTION","ZERO"].map(name=>(
                <button key={name} type="button" onClick={()=>applyPackage(name)} style={{height:39,borderRadius:11,border:`1px solid ${packageName===name?C.orange:C.border}`,background:packageName===name?"rgba(255,36,0,.14)":"rgba(255,255,255,.045)",color:packageName===name?C.orange:"#bbb",fontFamily:"'Space Mono',monospace",fontSize:10,fontWeight:900,letterSpacing:".12em",cursor:"pointer"}}>
                  {name}
                </button>
              ))}
            </div>
        ...
        <Txt label="Objetivo / argumento comercial" value={objective} onChange={setObjective} rows={3} placeholder="Explique a transformação que o vídeo precisa entregar."/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 16px"}}>
          <Txt label="Entregáveis" value={deliverables} onChange={setDeliverables} rows={6} placeholder={"Vídeo principal\n3 cortes para redes\nColor grading\nEntrega em formatos finais"}/>
          <Txt label="Cronograma" value={timeline} onChange={setTimeline} rows={6} placeholder={"Briefing\nProdução\nEdição\nRevisão\nEntrega"}/>
        </div>
      </Card>
...
const TabGoals = ({state,dispatch})=>{
  ...
  const levels=[{k:"annual",l:"Anual",c:C.orange},{k:"quarterly",l:"Trimestral",c:"#fb923c"},{k:"monthly",l:"Mensal",c:"#cc1d00"}];
...
const TabClients = ({state,dispatch,privacyMode})=>{
  ...
  const [detailClientId,setDetailClientId]=useState(null),[dragClientId,setDragClientId]=useState(null),[dragOverCol,setDragOverCol]=useState(null);
  ...
  const clients=state.clients||[],client=clients.find(c=>c.id===selected),detailClient=clients.find(c=>String(c.id)===String(detailClientId));
  const totalReceivable=clients.filter(c=>c.payment!=="pago").reduce((a,c)=>a+Number(c.value||0),0);
  const isFollowPending=c=>c.followUpDate&&dayDiff(c.followUpDate)<=0&&normalizeClientStatus(c)!=="pago";
  ...
  const selectRelationshipType=type=>setCf(f=>({...f,relationshipType:type,status:type==="freelancer"?"em_producao":f.status,payment:type==="parceria"?"pendente":f.payment}));
  const applyClientQuickStart=type=>{
    const presets={
      ...
      freelancer:{relationshipType:"freelancer",status:"em_producao",payment:"pendente",leadTemp:"quente",probability:80,leadSource:"Networking",service:"Freelancer audiovisual",freelancerRole:"Editor / filmmaker",nextAction:"Confirmar disponibilidade",followUpDate:addDaysInput(1)}
    };
  ...
  const removeClient=c=>{
    ...
    if(String(detailClientId)===String(c.id))setDetailClientId(null);
  };
  const openClientWindow=id=>{setSelected(null);setDetailClientId(id);};
  const editFromWindow=c=>{
    if(!c)return;
    setCf({...E,name:c.name,service:c.service||"",value:c.value||"",status:normalizeClientStatus(c),payment:c.payment,contract:c.contract||"",nextMeeting:c.nextMeeting||"",email:c.email||"",phone:c.phone||"",notes:c.notes||"",nextAction:c.nextAction||"",followUpDate:c.followUpDate||"",leadTemp:c.leadTemp||"morno",leadSource:c.leadSource||"",probability:c.probability??50,relationshipType:relationType(c),monthlyValue:c.monthlyValue||"",barterDetails:c.barterDetails||"",partnerTerms:c.partnerTerms||"",freelancerRole:c.freelancerRole||"",freelancerRate:c.freelancerRate||"",availability:c.availability||"",pix:c.pix||"",portfolio:c.portfolio||""});
    setEditClient(c.id);
    setShowAdd(true);
  };
  const moveDraggedClient=status=>{
    if(!dragClientId||!status)return;
    dispatch({type:"UPDATE_CLIENT",id:dragClientId,data:{status,...(status==="pago"?{payment:"pago"}:{})}});
    setDragClientId(null);
    setDragOverCol(null);
  };
  const renderClientWindow=c=>{
    if(!c)return null;
    const pv=(c.videos||[]).filter(v=>v.status!=="entregue").length;
    const rel=RELATIONSHIP_TYPES.find(r=>r.id===relationType(c))||RELATIONSHIP_TYPES[1];
    return (
      <div>
        <div style={{display:"flex",justifyContent:"space-between",gap:16,alignItems:"flex-start",marginBottom:18,flexWrap:"wrap"}}>
          <div style={{minWidth:0}}>
            <div style={{display:"flex",gap:7,flexWrap:"wrap",marginBottom:10}}>
              <Tag color={STATUS_COLORS[normalizeClientStatus(c)]||C.orange}>{clientStatusLabel(normalizeClientStatus(c))}</Tag>
            ...
            <div className="private-data" style={{fontSize:24,fontWeight:900,color:"#fff",fontFamily:"'Syne',sans-serif",lineHeight:1.05}}>{c.name}</div>
            <div style={{fontSize:13,color:C.muted,marginTop:6,lineHeight:1.45}}>{c.service||relationMeta(c)}</div>
          </div>
          <div style={{textAlign:"right"}}>
            <div style={{fontSize:24,fontWeight:900,color:"#10b981",fontFamily:"'Syne',sans-serif"}}>{fmtMoney(c.value,privacyMode)}</div>
            <div style={{fontSize:11,color:C.muted}}>prev. {fmtMoney(forecast(c),privacyMode)} · {c.probability??50}%</div>
          </div>
        </div>
        <div className="mobile-two-grid" style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:10,marginBottom:16}}>
          {c.phone&&<div className="client-window-field"><span>WhatsApp</span><b className="private-data">{c.phone}</b></div>}
          {c.email&&<div className="client-window-field"><span>Email</span><b className="private-data">{c.email}</b></div>}
          {c.nextAction&&<div className="client-window-field"><span>Próxima ação</span><b>{c.nextAction}</b></div>}
          {c.followUpDate&&<div className="client-window-field"><span>Follow-up</span><b>{new Date(c.followUpDate+"T00:00").toLocaleDateString("pt-BR")}</b></div>}
          {c.nextMeeting&&<div className="client-window-field"><span>Reunião</span><b>{new Date(c.nextMeeting+"T00:00").toLocaleDateString("pt-BR")}</b></div>}
          <div className="client-window-field"><span>Projetos</span><b>{pv} aberto{pv===1?"":"s"}</b></div>
        </div>
        {c.notes&&<div style={{fontSize:13,color:"#bbb",lineHeight:1.55,padding:"13px 14px",border:`1px solid ${C.border}`,borderRadius:12,background:"rgba(255,255,255,.035)",marginBottom:16}}>{c.notes}</div>}
        <div className="mobile-actions" style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:8}}>
          <Btn onClick={()=>editFromWindow(c)} variant="ghost" style={{justifyContent:"center"}}>Editar</Btn>
          <Btn onClick={()=>dispatch({type:"UPDATE_CLIENT",id:c.id,data:{payment:c.payment==="pago"?"pendente":"pago"}})} variant={c.payment==="pago"?"ghost":"success"} style={{justifyContent:"center"}}>{c.payment==="pago"?"Pendente":"Pago"}</Btn>
          <Btn onClick={()=>{setDetailClientId(null);setSelected(c.id);}} variant="proposal" style={{justifyContent:"center"}}>Abrir completo</Btn>
        </div>
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:10}}>
          {pipeline.map(p=><button key={p.key} onClick={()=>dispatch({type:"UPDATE_CLIENT",id:c.id,data:{status:p.key}})} style={{padding:"7px 10px",borderRadius:9,border:"1px solid",borderColor:normalizeClientStatus(c)===p.key?p.color:C.border,background:normalizeClientStatus(c)===p.key?`${p.color}16`:"rgba(255,255,255,.025)",color:normalizeClientStatus(c)===p.key?p.color:C.muted,fontSize:10,fontWeight:900,cursor:"pointer",fontFamily:"inherit",textTransform:"uppercase"}}>{p.label}</button>)}
        </div>
      </div>
    );
  };
  const renderClientSmartForm=()=>(
    <>
      ...
      <div className="client-modal-controls">
        <div style={{marginBottom:13}}><div style={{fontSize:11,color:C.muted,marginBottom:6,fontWeight:700,textTransform:"uppercase"}}>Status</div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{Object.entries(STATUS_COLORS).map(([k,c])=><button key={k} onClick={()=>setCf(f=>({...f,status:k}))} style={{padding:"5px 11px",borderRadius:8,border:"1px solid",borderColor:cf.status===k?c:C.border,background:cf.status===k?`${c}15`:"transparent",color:cf.status===k?c:C.muted,fontSize:11,fontWeight:700,cursor:"pointer"}}>{clientStatusLabel(k)}</button>)}</div></div>
      ...
      <div>
        ...
        <Card style={{background:`${STATUS_COLORS[normalizeClientStatus(client)]||C.orange}08`,borderColor:`${STATUS_COLORS[normalizeClientStatus(client)]||C.orange}25`,marginBottom:14}}>
          <div className="client-detail-head">
            <div><div style={{display:"flex",gap:8,alignItems:"center",marginBottom:8,flexWrap:"wrap"}}><Tag color={STATUS_COLORS[normalizeClientStatus(client)]||C.orange}>{clientStatusLabel(normalizeClientStatus(client))}</Tag><Tag color={PAG_COLORS[client.payment]||C.orange}>{client.payment}</Tag><Tag color={TEMP_COLORS[client.leadTemp]||"#eab308"}>{client.leadTemp||"morno"}</Tag><Tag color={(RELATIONSHIP_TYPES.find(r=>r.id===relationType(client))||RELATIONSHIP_TYPES[1]).color}>{(RELATIONSHIP_TYPES.find(r=>r.id===relationType(client))||RELATIONSHIP_TYPES[1]).label}</Tag>{isFollowPending(client)&&<Tag color="#ef4444">follow-up</Tag>}</div><div className="private-data" style={{fontSize:20,fontWeight:800,color:"#fff",fontFamily:"'Syne',sans-serif"}}>{client.name}</div>{client.service&&<div style={{fontSize:13,color:C.muted,marginTop:3}}>{client.service}</div>}</div>
  ...
  return (
    <div>
      <div className="mobile-kpi-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:16}}>
        <Card style={{padding:"14px 16px",textAlign:"center"}}><div style={{fontSize:24,fontWeight:800,color:"#10b981",fontFamily:"'Syne',sans-serif"}}>{clients.filter(c=>normalizeClientStatus(c)==="em_producao").length}</div><div style={{fontSize:10,color:C.muted,marginTop:2}}>Em produção</div></Card>
      ...
      {view==="pipeline"&&filteredClients.length>0&&(
        <div className="pipeline-board" style={{marginBottom:16}}>
          {pipeline.map(col=>{
            const items=filteredClients.filter(c=>normalizeClientStatus(c)===col.key||(col.key==="lead"&&!pipelineKeys.includes(normalizeClientStatus(c))));
            ...
            return (
              <div
                ...
                onDrop={e=>{e.preventDefault();moveDraggedClient(col.key);}}
                style={{background:dragOverCol===col.key?`${col.color}10`:"rgba(255,255,255,.025)",border:`1px solid ${dragOverCol===col.key?`${col.color}66`:C.border}`,borderRadius:14,padding:10,minHeight:160,transition:"background .15s,border-color .15s"}}
              >
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                ...
                {items.map(c=>{
                  ...
                  return (
                    <div
                      key={c.id}
                      className="card-hover client-drag-card"
                      draggable
                      onDragStart={e=>{setDragClientId(c.id);e.dataTransfer.effectAllowed="move";e.dataTransfer.setData("text/plain",String(c.id));}}
                      onDragEnd={()=>{setDragClientId(null);setDragOverCol(null);}}
                      onClick={()=>openClientWindow(c.id)}
                      style={{background:String(dragClientId)===String(c.id)?"rgba(255,36,0,.09)":"rgba(255,255,255,.04)",border:`1px solid ${col.color}22`,borderRadius:12,padding:"12px 12px",marginBottom:8,cursor:"grab",opacity:String(dragClientId)===String(c.id)?0.72:1}}
                    >
                      ...
                      {c.nextAction&&<div style={{fontSize:11,color:"#bbb",lineHeight:1.35}}>Próxima ação: {c.nextAction}</div>}
                      <div className="client-card-actions" onClick={e=>e.stopPropagation()} onPointerDown={e=>e.stopPropagation()}>
                        <button title="Abrir cliente" onClick={()=>openClientWindow(c.id)} aria-label="Abrir cliente">↗</button>
                        <button title="Editar cliente" onClick={()=>editFromWindow(c)} aria-label="Editar cliente">✎</button>
                        <button title={c.payment==="pago"?"Marcar pendente":"Marcar pago"} onClick={()=>dispatch({type:"UPDATE_CLIENT",id:c.id,data:{payment:c.payment==="pago"?"pendente":"pago"}})} aria-label="Alterar pagamento">$</button>
                        <button title="Excluir cliente" onClick={()=>removeClient(c)} aria-label="Excluir cliente" className="danger">×</button>
                      </div>
                      <div style={{fontSize:9,color:C.muted,marginTop:8,textTransform:"uppercase",letterSpacing:".12em",fontWeight:900}}>Arraste para mudar etapa</div>
                    </div>
                  );
                })}
              </div>
      ...
      {view==="list"&&filteredClients.map(c=>{
        ...
        const dtm=c.nextMeeting?Math.ceil((new Date(c.nextMeeting)-new Date())/(1000*60*60*24)):null;
        return (
          <Card key={c.id} onClick={()=>openClientWindow(c.id)} style={{marginBottom:10,cursor:"pointer"}}>
      ...
      <Modal open={showAdd} onClose={()=>{setShowAdd(false);setEditClient(null);setCf(E);}} title={editClient?"Editar Cliente":"Novo Cliente"} wide>
        {renderClientSmartForm()}
      </Modal>
      <Modal open={!!detailClient} onClose={()=>setDetailClientId(null)} title="Janela do cliente" wide>
        {renderClientWindow(detailClient)}
      </Modal>
    </div>
  );
};
...
const TabExport = ({state,dispatch})=>{
  ...
  const REPORTS=[
    {id:"overview",icon:"▦",title:"Resumo executivo",desc:"Indicadores da operação audiovisual da DNZ Films",color:C.orange},
    ...
    {id:"review",icon:"◉",title:"Video Review",desc:"Links enviados, aprovações e ajustes solicitados",color:"#06b6d4"},
  ];
  ...
  const exportBackup=()=>{
    ...
    a.href=url;
    a.download=`dnz-central-backup-${year}-${String(month+1).padStart(2,"0")}.json`;
    a.click();
  ...
  const exportEncryptedBackup=async()=>{
    try{
      ...
      const a=document.createElement("a");
      a.href=url;
      a.download=`dnz-central-backup-criptografado-${year}-${String(month+1).padStart(2,"0")}.json`;
      a.click();
      URL.revokeObjectURL(url);
      const now=new Date().toISOString();
      localStorage.setItem("dcc_last_backup",now);
  ...
  const generatePDF=()=>{
    ...
    const clientsSection=(state.clients||[]).map(c=>{const ints=(c.interactions||[]).filter(i=>{const p=i.date?.split("/");return p?.length===3&&parseInt(p[1])-1===month&&parseInt(p[2])===year;});return `<div style="margin-bottom:12px;padding:12px 16px;border:1px solid #333;border-radius:10px;background:#1a1a1a"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px"><strong style="color:#fff;font-size:15px">${esc(c.name)}</strong><span style="color:#10b981;font-weight:800">${fmtCurrency(c.value)}</span></div><div style="font-size:11px;color:#888;margin-bottom:8px">${esc(c.service||"")} · ${esc(c.status)} · ${esc(c.payment)}</div>${ints.length?`<div style="font-size:11px;color:#ff2400;font-weight:700;margin-bottom:4px">INTERAÇÕES (${ints.length})</div>${ints.map(i=>`<div style="font-size:12px;color:#aaa;padding:4px 0;border-left:2px solid #ff2400;padding-left:8px;margin:3px 0">[${esc(i.type)}] ${esc(i.note)}</div>`).join("")}`:""}</div>`;}).join("");
    const projects=(state.clients||[]).flatMap(c=>(c.videos||[]).map(v=>({client:c,video:v})));
    const activeProjects=projects.filter(p=>p.video.status!=="entregue");
    const docs=state.studioDocs||[];
    const reviews=state.reviewDeliverables||[];
    const productionHTML=hasReport("production")?`<h2>▶ Produção</h2><div style="margin-bottom:16px">${tasksSection}</div>${activeProjects.map(p=>`<div style="margin-bottom:12px;padding:12px 16px;border:1px solid #333;border-radius:10px;background:#1a1a1a"><strong style="color:#fff">${esc(p.video.title)}</strong><div style="font-size:12px;color:#888;margin-top:4px">${esc(p.client.name)} · ${esc(p.video.status||"em produção")} · prazo ${esc(p.video.deadline||"sem data")}</div></div>`).join("")||`<div style="color:#555">Nenhum projeto ativo.</div>`}<h2>▣ Documentos gerados</h2>${docs.slice(0,12).map(d=>`<div style="font-size:13px;color:#ccc;padding:8px 0;border-bottom:1px solid #222">${esc(d.docLabel||"Documento")} · ${esc(d.title||"sem título")} · ${esc(d.clientName||"sem cliente")}</div>`).join("")||`<div style="color:#555">Nenhum documento salvo.</div>`}`:"";
    const reviewStatusLabel=s=>s==="approved"?"Aprovado":s==="revision_requested"?"Ajustes":s==="approved_with_changes"?"Ajustes leves":s==="rejected"?"Refazer":"Aguardando";
    const videoReviewHTML=hasReport("review")?`<h2>◉ Video Review</h2>${reviews.map(r=>`<div style="margin-bottom:12px;padding:12px 16px;border:1px solid #333;border-radius:10px;background:#1a1a1a"><div style="display:flex;justify-content:space-between;gap:10px"><strong style="color:#fff">${esc(r.title)}</strong><span style="color:#ff2400;font-weight:800">${esc(reviewStatusLabel(r.status))}</span></div><div style="font-size:12px;color:#888;margin-top:4px">${esc(r.project_title||"sem projeto")} · ${esc(r.client_name||"sem cliente")} · ${(r.comments||[]).length} comentários</div></div>`).join("")||`<div style="color:#555">Nenhum review publicado.</div>`}`:"";
    const revenue=(state.clients||[]).reduce((a,c)=>a+Number(c.value||0),0);
    const received=(state.clients||[]).filter(c=>c.payment==="pago").reduce((a,c)=>a+Number(c.value||0),0);
    ...
    const overviewHTML=hasReport("overview")?`<h2>▦ Resumo executivo</h2><div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:18px"><div class="kpi"><div class="kpi-val">${projects.length}</div><div class="kpi-label">Projetos</div></div><div class="kpi"><div class="kpi-val">${reviews.length}</div><div class="kpi-label">Reviews</div></div><div class="kpi"><div class="kpi-val">${docs.length}</div><div class="kpi-label">Docs</div></div><div class="kpi"><div class="kpi-val" style="color:#10b981">${fmtCurrency(received)}</div><div class="kpi-label">Recebido</div></div></div>`:"";
    const commercialHTML=hasReport("commercial")&&state.clients?.length?`<h2>🤝 Clientes</h2>${clientsSection}`:"";
    const reviewHTML=hasReport("review")&&weekRevs.length?`<h2>📋 Revisões Semanais</h2>${weekRevs.map(([k,r])=>`<div style="margin-bottom:12px;padding:14px;background:#1a1a1a;border:1px solid #333;border-radius:10px"><div style="color:#ff2400;font-weight:700;font-size:12px;margin-bottom:8px">${esc(k)}</div>${r.done?`<div style="margin-bottom:6px"><strong style="color:#ccc">Feito:</strong><p style="color:#aaa;font-size:13px;margin-top:3px">${esc(r.done)}</p></div>`:""}${r.learned?`<div style="margin-bottom:6px"><strong style="color:#ccc">Aprendizados:</strong><p style="color:#aaa;font-size:13px;margin-top:3px">${esc(r.learned)}</p></div>`:""}${r.improve?`<div style="margin-bottom:6px"><strong style="color:#ccc">Melhorias:</strong><p style="color:#aaa;font-size:13px;margin-top:3px">${esc(r.improve)}</p></div>`:""}${r.prioritize?`<div><strong style="color:#ccc">Prioridades:</strong><p style="color:#aaa;font-size:13px;margin-top:3px">${esc(r.prioritize)}</p></div>`:""}</div>`).join("")}`:"";
    const emptyHTML=!overviewHTML&&!productionHTML&&!commercialHTML&&!videoReviewHTML&&!reviewHTML?`<div style="padding:22px;background:#1a1a1a;border:1px solid #333;border-radius:12px;color:#888;text-align:center">Nenhum dado encontrado para os relatórios selecionados nesse período.</div>`:"";
    const logoSrc=business.logoUrl||DNZ_LOGO_URL;
    const html=`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${esc(reportName)} — ${MONTHS[month]} ${year}</title><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Bebas+Neue&family=DM+Sans:wght@400;600;700;900&family=Space+Mono:wght@400;700&display=swap" rel="stylesheet"><style>*{box-sizing:border-box;margin:0;padding:0}body{background:#0d0d0d;color:#e8e8e8;font-family:'DM Sans','Segoe UI',Arial,sans-serif;padding:40px;max-width:900px;margin:0 auto}h1{font-family:'Bebas Neue',Impact,sans-serif;font-size:48px;line-height:.92;font-weight:900;color:#fff;margin-bottom:8px;letter-spacing:0}h2{font-family:'Space Mono',monospace;font-size:13px;font-weight:800;color:#ff2400;text-transform:uppercase;letter-spacing:.1em;margin:28px 0 14px;padding-bottom:8px;border-bottom:1px solid #333}.hero{background:linear-gradient(135deg,rgba(255,36,0,.12),rgba(0,0,0,0));border:1px solid rgba(255,36,0,.25);border-radius:16px;padding:24px 28px;margin-bottom:28px}.hero-brand{display:flex;align-items:center;gap:14px;margin-bottom:16px}.hero-logo{width:54px;height:54px;object-fit:contain;display:block}.hero-kicker{font-family:'Space Mono',monospace;font-size:12px;color:#ff2400;font-weight:700;letter-spacing:.1em;text-transform:uppercase}.kpi-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:28px}.kpi{background:#1a1a1a;border:1px solid #2a2a2a;border-radius:12px;padding:16px;text-align:center}.kpi-val{font-size:24px;font-weight:900;color:#ff2400}.kpi-label{font-size:11px;color:#666;margin-top:4px;text-transform:uppercase;letter-spacing:.06em}@media print{body{padding:20px}}</style></head><body>
    <div class="hero"><div class="hero-brand"><img class="hero-logo" src="${esc(logoSrc)}" alt="${esc(business.brandName||APP_NAME)}"><div class="hero-kicker">DNZ FILMS — DOES NOT ZERO</div></div><h1>${esc(reportName)}</h1><div style="color:#666;font-size:14px;margin-bottom:16px">${MONTHS[month]} de ${year} · ${today.toLocaleDateString("pt-BR",{weekday:"long",day:"numeric",month:"long",year:"numeric"})}</div><div style="display:flex;gap:12px;flex-wrap:wrap"><span style="background:rgba(255,36,0,.15);border:1px solid rgba(255,36,0,.3);padding:4px 12px;border-radius:99px;font-size:12px;font-weight:700;color:#ff2400">Receita mapeada: ${fmtCurrency(revenue)}</span><span style="background:rgba(255,36,0,.15);border:1px solid rgba(255,36,0,.3);padding:4px 12px;border-radius:99px;font-size:12px;font-weight:700;color:#ff2400">${projects.length} projetos</span></div></div>
    ${overviewHTML}${productionHTML}${commercialHTML}${videoReviewHTML}${reviewHTML}${emptyHTML}
  ...
  const activeClients=(state.clients||[]).filter(c=>normalizeClientStatus(c)==="em_producao").length;
  const paidTotal=(state.clients||[]).filter(c=>c.payment==="pago").reduce((a,c)=>a+Number(c.value||0),0);
  const allProjects=(state.clients||[]).flatMap(c=>c.videos||[]);
  const activeProjects=allProjects.filter(v=>v.status!=="entregue").length;
  const reviewCount=(state.reviewDeliverables||[]).length;
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:16}}>
      <Card style={{background:`${C.orange}08`,borderColor:`${C.orange}20`,padding:"20px 22px"}}>
        ...
        <p style={{margin:0,fontSize:14,color:"#aaa",lineHeight:1.6}}>Relatórios internos da DNZ Films: produção, clientes, propostas, Video Review, recebimentos e backup da operação.</p>
      </Card>
      <Card style={{padding:"20px 22px"}}>
        ...
        <div className="mobile-kpi-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:18}}>
          {[
            ...
            {v:reviewCount,l:"Video Reviews",c:"#06b6d4"},
            {v:activeClients,l:"Em produção",c:"#8b5cf6"},
        ...
        <div style={{display:"flex",gap:8,flexWrap:"wrap",margin:"-2px 0 14px"}}>
          {[
            ...
            ["Comercial","#10b981"],
            ["Review","#06b6d4"],
          ].map(([label,color])=><span key={label} style={{border:`1px solid ${color}44`,background:`${color}12`,color, borderRadius:999,padding:"5px 9px",fontSize:10,fontWeight:900,letterSpacing:".08em",textTransform:"uppercase"}}>{label}</span>)}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr",gap:9,marginBottom:16}}>
          {REPORTS.map(r=>{
            const on=hasReport(r.id);
            return <button key={r.id} onClick={()=>toggleReport(r.id)} style={{display:"flex",alignItems:"center",gap:11,textAlign:"left",padding:"10px 12px",borderRadius:10,border:`1px solid ${on?`${r.color}44`:C.border}`,borderLeft:`3px solid ${on?r.color:"rgba(255,255,255,.12)"}`,background:on?`linear-gradient(90deg,${r.color}12,rgba(255,255,255,.025))`:"rgba(255,255,255,.025)",color:"#fff",cursor:"pointer",fontFamily:"inherit",transition:"all .18s"}}>
              <span style={{fontSize:15,width:22,textAlign:"center",color:on?r.color:"#aaa"}}>{r.icon}</span>
              <span style={{flex:1,minWidth:0}}><span style={{display:"block",fontSize:12,fontWeight:900,color:on?r.color:"#e5e5e5"}}>{r.title}</span><span style={{display:"block",fontSize:10,color:C.muted,marginTop:2,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{r.desc}</span></span>
              <span style={{fontSize:15,color:on?r.color:C.muted}}>{on?"✓":"+"}</span>
            </button>;
          })}
        </div>
...
const SalesTrialNotice = ({session,userName,remaining,onClose,onAbout})=>{
  ...
  const accessMessage=`Olá, preciso liberar meu acesso interno ao ${APP_NAME}. Meu login é ${session?.user?.email||"ainda não entrei"}.`;
  const accessHref=`https://wa.me/${SALES_WHATSAPP}?text=${encodeURIComponent(accessMessage)}`;
  const supportHref=`https://wa.me/${SALES_WHATSAPP}?text=${encodeURIComponent(`Olá, preciso de suporte no ${APP_NAME}.`)}`;
  return (
    <div className={`sales-float ${expired?"locked":""}`} role="dialog" aria-live="polite">
      <div className="sales-float-head">
        <div>
          <div style={{fontSize:10,color:C.orange,fontWeight:900,letterSpacing:".14em",textTransform:"uppercase",marginBottom:5}}>{expired?"Sessão expirada":logged?"Acesso privado":"Área interna"}</div>
          <div style={{fontSize:17,color:"#fff",fontWeight:900,fontFamily:"'Syne',sans-serif",lineHeight:1.15}}>{logged?`${userName||"Usuário"}, sessão protegida`:`Entrar no ${APP_NAME}`}</div>
        </div>
      ...
      <div className="sales-float-body">
        {logged?(
          <>
            ...
            <p style={{fontSize:13,color:"#bbb",lineHeight:1.5,margin:"0 0 13px"}}>{expired?"A sessão foi bloqueada por segurança. Libere novamente com o admin ou faça login de novo.":"Sessão temporária ativa. O acesso completo depende da allowlist de admin no Supabase."}</p>
          </>
        ):(
          <p style={{fontSize:13,color:"#bbb",lineHeight:1.5,margin:"0 0 13px"}}>O login identifica quem está entrando e bloqueia qualquer e-mail fora da allowlist interna.</p>
        )}
        <div style={{display:"grid",gridTemplateColumns:logged?"1fr 1fr":"1fr",gap:8}}>
          <a href={accessHref} target="_blank" rel="noopener noreferrer" style={{display:"inline-flex",alignItems:"center",justifyContent:"center",minHeight:38,borderRadius:11,background:`linear-gradient(135deg,${C.orange},${C.orangeD})`,color:"#fff",textDecoration:"none",fontSize:12,fontWeight:900}}>Solicitar liberação</a>
        ...
        <button onClick={onAbout} style={{width:"100%",marginTop:8,minHeight:34,borderRadius:10,border:`1px solid ${C.border}`,background:"rgba(255,255,255,.035)",color:"#ddd",fontFamily:"inherit",fontSize:12,fontWeight:900,cursor:"pointer"}}>Ver mapa interno</button>
        <div style={{fontSize:10,color:C.muted,marginTop:10,lineHeight:1.35}}>Acesso privado DNZ Films. GitHub autentica; Supabase valida e sincroniza os dados.</div>
      </div>
    </div>
  );
};
...
const TabAbout = ({session,onEnter,setTab})=>{
  const [showDetails,setShowDetails]=useState(false);
  const features=[
    {k:"01",title:"Entrada",text:"Cliente, contexto, briefing, prioridade e próxima ação no mesmo lugar.",color:"#10b981"},
    {k:"02",title:"Produção",text:"Projeto com etapas, prazos, arquivos, responsáveis e checklist de set.",color:"#8b5cf6"},
    {k:"03",title:"Revisão",text:"Video Review com link público, comentários por timecode e aprovação clara.",color:"#ff2400"},
    {k:"04",title:"Entrega",text:"Proposta, PDF, cobrança e histórico conectados ao cliente certo.",color:"#3b82f6"},
  ];
  const flow=[
    ["Briefing","O que entrou, quem pediu e qual entrega precisa sair."],
    ["Projeto","Prazo, etapa, arquivos, checklist e produção ativa."],
    ["Review","Cliente comenta no vídeo, aprova ou pede ajuste."],
    ["Entrega","PDF, cobrança, aceite e histórico fechados."],
  ];
  return (
    <div className="page-stack">
      <section className="elite-about-hero elite-surface">
        <div className="elite-content" style={{display:"flex",flexDirection:"column",justifyContent:"space-between",gap:24}}>
          <div>
            <div className="elite-kicker">DOES NOT ZERO</div>
            <h1 className="elite-title">Central interna<br/><span style={{color:C.orange}}>da operação criativa</span></h1>
            <p className="elite-copy">Este é o mapa privado da DNZ Films: o lugar para enxergar clientes, projetos, revisões, propostas e recebimentos sem espalhar a operação em abas soltas.</p>
          </div>
          <div>
            <div className="elite-actions">
              <button onClick={()=>setTab?.("today")} className="elite-primary">Abrir hoje</button>
              <button onClick={()=>setTab?.("videoReview")} className="elite-secondary">Video Review</button>
              <button onClick={onEnter} className="elite-secondary">{session?"Continuar operação":"Entrar"}</button>
            </div>
            <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:18}}>
              {["Clientes","Projetos","Review","Propostas","Financeiro"].map(item=><span key={item} style={{fontSize:11,color:"#aaa",fontWeight:900,border:"1px solid rgba(255,255,255,.1)",background:"rgba(255,255,255,.045)",borderRadius:99,padding:"6px 10px"}}>{item}</span>)}
            </div>
          </div>
        </div>
        <div className="elite-content elite-product-panel">
          ...
          <div className="elite-mini-metrics">
            <div className="elite-mini-metric"><div className="elite-mini-value">CRM</div><div className="elite-mini-label">clientes ativos</div></div>
            <div className="elite-mini-metric"><div className="elite-mini-value">REV</div><div className="elite-mini-label">aprovação de vídeo</div></div>
            <div className="elite-mini-metric"><div className="elite-mini-value">PDF</div><div className="elite-mini-label">propostas e docs</div></div>
          </div>
        </div>
      </section>
      <div className="plain-intro-grid">
        <div className="plain-intro-card">
          <div className="plain-intro-kicker">Função</div>
          <div className="plain-intro-title">Operar a produtora com menos ruído.</div>
          <p className="plain-intro-text">O sistema existe para saber o que entrou, o que está em produção, o que precisa de revisão e o que falta receber.</p>
        </div>
        <div className="plain-intro-card">
          <div className="plain-intro-kicker">Ritual diário</div>
          <div className="plain-intro-title">Abrir, decidir, executar.</div>
          <div className="plain-intro-list">
            <span><b>1.</b> Ver pendências do dia.</span>
            <span><b>2.</b> Mover clientes e projetos.</span>
            <span><b>3.</b> Enviar review, proposta ou cobrança.</span>
          </div>
        </div>
        <div className="plain-intro-card">
          <div className="plain-intro-kicker">Prioridade</div>
          <div className="plain-intro-title">Video Review como núcleo.</div>
          <p className="plain-intro-text">A revisão de vídeo passa a ser o ponto principal entre produção, cliente, aprovação e entrega final.</p>
        </div>
      </div>
      <Card>
        <SectionTitle>ATALHOS OPERACIONAIS</SectionTitle>
        <div style={{display:"grid",gridTemplateColumns:"repeat(5,minmax(0,1fr))",gap:12}} className="modal-grid">
          {[
            ["Hoje","Prioridades, atrasos e próxima ação."],
            ["Clientes","Pipeline comercial e posição de cada contato."],
            ["Projetos","Produção, etapas, prazos e arquivos."],
            ["Review","Vídeo, timecode, comentários e aprovação."],
            ["Propostas","PDF comercial ligado ao cliente."],
      ...
      <Card style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:16,flexWrap:"wrap",padding:"16px 18px"}}>
        <div>
          <div style={{fontSize:14,color:"#fff",fontWeight:900,marginBottom:3}}>Detalhes recolhidos por padrão</div>
          <div style={{fontSize:12,color:C.muted}}>A tela fica limpa para operar. O mapa completo aparece só quando precisar consultar.</div>
        </div>
        <Btn onClick={()=>setShowDetails(v=>!v)} variant="ghost">{showDetails?"Ocultar detalhes":"Ver mapa completo"}</Btn>
      </Card>
      {showDetails&&<>
      <div className="elite-section-title">
        <div>
          <h2>Fluxo interno</h2>
          <p>O pedido vira projeto, o projeto vira revisão, a revisão vira aprovação e a entrega fecha o histórico.</p>
        </div>
      </div>
      <Card style={{padding:"18px",background:"rgba(255,36,0,.06)",borderColor:"rgba(255,36,0,.2)"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,minmax(0,1fr))",gap:10}}>
          {[
            ["01","Capturar","Registrar cliente, briefing, origem e próxima ação."],
            ["02","Produzir","Organizar etapas, prazo, arquivos e checklist."],
            ["03","Revisar","Enviar link público, coletar timecodes e aprovar."],
            ["04","Fechar","Gerar PDF, entregar, cobrar e guardar histórico."],
      ...
      <Card className="elite-surface" style={{padding:"20px"}}>
        <div className="elite-content">
          <div className="elite-section-title">
            <div>
              <h2>Padrão DNZ</h2>
              <p>Visual escuro, direto e cinematográfico para a operação interna da produtora.</p>
            </div>
          </div>
          <div className="elite-access-grid">
            {[
              ["Logo atual","Aplicada na navegação, proposta, review e documentos."],
              ["Tom interno","Textos escritos para operação, não para vender assinatura."],
              ["Video Review","Produto principal do fluxo entre cliente e entrega."],
              ["Histórico","Cliente, projeto, proposta e aprovação ficam conectados."],
    ...
    <div className="page-stack">
      ...
      <div className="mobile-two-grid" style={{display:"grid",gridTemplateColumns:"1.2fr .8fr",gap:12}}>
        <Card>
          ...
          {[
            ["Sobre","Primeiro o usuário entende a promessa da DNZ Central."],
        ...
        <Card style={{background:"rgba(255,36,0,.06)",borderColor:"rgba(255,36,0,.22)"}}>
...
const TabTemplates = ({state,dispatch,setTab})=>{
  const [selected,setSelected]=useState(null);
  const taskTemplates=[
    {id:"followup",kind:"tasks",title:"Follow-up comercial",tag:"crm",desc:"Fila curta para responder lead, proposta e retorno de orçamento.",items:["Revisar leads quentes","Enviar proposta pendente","Cobrar retorno de orçamento","Atualizar probabilidade no CRM"],target:"Atividades"},
    {id:"preproducao",kind:"tasks",title:"Pré-produção audiovisual",tag:"produção",desc:"Checklist mínimo antes de sair para captar.",items:["Confirmar briefing","Fechar roteiro","Separar referências","Montar plano de captação"],target:"Atividades"},
    {id:"financeiro",kind:"tasks",title:"Fechamento financeiro",tag:"financeiro",desc:"Rotina semanal para manter recebimentos e despesas claros.",items:["Conferir recebidos","Atualizar atrasados","Registrar despesas","Projetar próxima semana"],target:"Financeiro"},
  ];
  const projectTemplates=AUDIOVISUAL_PRESETS.map(p=>({id:`project-${p.id}`,kind:"project",presetId:p.id,title:p.title,type:p.type,desc:p.desc||`Projeto ${p.type} com checklist e entregáveis pré-montados.`,checklist:p.checklist,target:"Projetos"}));
  const playbookTemplates=NICHE_PLAYBOOKS.map(pb=>({...pb,id:`playbook-${pb.id}`,kind:"playbook",title:`${pb.service} · ${pb.niche}`,desc:pb.promise,target:"Cliente + Projeto + Atividades"}));
  const templates=[...projectTemplates,...taskTemplates,...playbookTemplates];
  ...
  const applyPlaybook=pb=>{
    ...
    const client={id:clientId,name:`Contato ${pb.niche}`,service:pb.service,value:pb.value,status:"lead",payment:"pendente",contract:"",nextMeeting:"",email:"",phone:"",notes:`Playbook aplicado: ${pb.niche}. ${pb.notes}`,nextAction:pb.nextAction,followUpDate:addDaysInput(2),leadTemp:"quente",leadSource:pb.leadSource,probability:65};
  ...
  const colorFor=t=>t.kind==="project"?"#8b5cf6":t.kind==="playbook"?(t.color||"#10b981"):C.orange;
  const itemsFor=t=>t.kind==="project"?(t.checklist||[]):t.kind==="playbook"?[...(t.offer||[]).slice(0,4),...(t.tasks||[]).slice(0,3)]:(t.items||[]);
  const apply=()=>{
    if(!selected)return;
    if(selected.kind==="tasks")addTasks(selected);
    if(selected.kind==="project")addProject(selected);
    if(selected.kind==="playbook")applyPlaybook(selected);
    setSelected(null);
  };
  const Column=({title,list,color})=>(
    <Card style={{padding:"16px"}}>
      ...
      <div style={{display:"grid",gap:8}}>
        {list.map(t=><button key={t.id} onClick={()=>setSelected(t)} style={{width:"100%",display:"grid",gridTemplateColumns:"1fr auto",gap:10,alignItems:"center",textAlign:"left",padding:"12px",borderRadius:12,border:`1px solid ${colorFor(t)}28`,background:`${colorFor(t)}0b`,color:"#ddd",cursor:"pointer",fontFamily:"inherit"}}>
          <span>
            <span style={{display:"block",fontSize:13,fontWeight:900,color:"#fff"}}>{t.title}</span>
            <span style={{display:"block",fontSize:11,color:C.muted,marginTop:4}}>{t.kind==="project"?`${t.checklist.length} etapas · ${t.type}`:t.kind==="playbook"?`${t.tasks.length} tarefas · ${fmtCurrency(t.value)}`:`${t.items.length} tarefas · ${t.tag}`}</span>
          </span>
          <span style={{fontSize:18,color:colorFor(t)}}>›</span>
        </button>)}
      </div>
    </Card>
  );
  return (
    <div className="page-stack">
      <Card className="page-hero" style={{padding:"20px 22px",background:"rgba(255,36,0,.05)",borderColor:"rgba(255,36,0,.2)"}}>
        <div className="page-hero-row">
          <div>
            <div className="page-eyebrow" style={{color:C.orange}}>BIBLIOTECA DNZ</div>
            <div className="page-title">Modelos de operação</div>
            <p className="page-subtitle">Escolha um modelo, revise em uma janela e aplique só quando fizer sentido. A biblioteca cria tarefas, projetos ou um fluxo completo de cliente.</p>
          </div>
          <Tag color="#10b981">{templates.length} modelos</Tag>
        </div>
      </Card>
      {!firstClient&&<Card style={{padding:"14px 16px",background:"rgba(234,179,8,.08)",borderColor:"rgba(234,179,8,.28)"}}>
        <div style={{fontSize:12,color:"#eab308",fontWeight:900}}>Para aplicar modelos de projeto, cadastre pelo menos um cliente. Modelos de tarefas funcionam agora.</div>
      </Card>}
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:12}} className="mobile-two-grid">
        <Column title="Projetos" list={projectTemplates}/>
        <Column title="Rotinas" list={taskTemplates}/>
        <Column title="Playbooks" list={playbookTemplates}/>
      </div>
      <Modal open={!!selected} onClose={()=>setSelected(null)} title={selected?.title||"Modelo"} wide>
        {selected&&<div style={{display:"grid",gap:14}}>
          <div className="modal-section primary">
            <div style={{display:"flex",justifyContent:"space-between",gap:14,alignItems:"flex-start",flexWrap:"wrap"}}>
              <div>
                <div style={{fontSize:10,color:colorFor(selected),fontWeight:900,letterSpacing:".14em",textTransform:"uppercase",marginBottom:6}}>{selected.target}</div>
                <div style={{fontSize:24,color:"#fff",fontWeight:900,fontFamily:"'Syne',sans-serif",lineHeight:1.05}}>{selected.title}</div>
                <p style={{fontSize:13,color:C.muted,lineHeight:1.55,margin:"8px 0 0",maxWidth:720}}>{selected.desc}</p>
              </div>
              <Tag color={colorFor(selected)}>{selected.kind==="playbook"?fmtCurrency(selected.value):selected.kind==="project"?selected.type:selected.tag}</Tag>
            </div>
          </div>
          <div className="modal-section">
            <SectionTitle>O QUE SERÁ CRIADO</SectionTitle>
            <div style={{display:"grid",gridTemplateColumns:"repeat(2,minmax(0,1fr))",gap:8}} className="modal-grid">
              {itemsFor(selected).map((item,i)=><div key={`${item}-${i}`} style={{display:"flex",gap:9,alignItems:"flex-start",padding:"10px 11px",borderRadius:12,border:`1px solid ${C.border}`,background:"rgba(255,255,255,.035)"}}>
                <span style={{color:colorFor(selected),fontWeight:900}}>✓</span>
                <span style={{fontSize:12,color:"#ddd",lineHeight:1.4}}>{typeof item==="string"?item:item.text}</span>
              </div>)}
            </div>
          </div>
          <div className="modal-section">
            <SectionTitle>DESTINO</SectionTitle>
            <div style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:10}} className="modal-grid">
              <div className="metric-tile"><div className="metric-value" style={{color:colorFor(selected)}}>{selected.kind==="project"?"Projeto":selected.kind==="playbook"?"Cliente":"Tarefas"}</div><div className="metric-label">Tipo</div></div>
              <div className="metric-tile"><div className="metric-value" style={{color:"#10b981"}}>{selected.kind==="project"?(firstClient?.name||"sem cliente"):selected.kind==="playbook"?"novo lead":"agenda"}</div><div className="metric-label">Destino</div></div>
              <div className="metric-tile"><div className="metric-value" style={{color:C.orange}}>{itemsFor(selected).length}</div><div className="metric-label">Itens</div></div>
            </div>
          </div>
          <div className="mobile-actions" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            <Btn variant="ghost" onClick={()=>setSelected(null)} style={{justifyContent:"center"}}>Cancelar</Btn>
            <Btn onClick={apply} disabled={selected.kind==="project"&&!firstClient} style={{justifyContent:"center",background:`linear-gradient(135deg,${colorFor(selected)},${C.orangeD})`}}>{selected.kind==="project"&&!firstClient?"Cadastre um cliente":"Aplicar modelo"}</Btn>
          </div>
        </div>}
      </Modal>
      <Card style={{padding:"14px 16px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr auto",gap:12,alignItems:"center"}}>
          <div>
            <div style={{fontSize:15,color:"#fff",fontWeight:900,fontFamily:"'Syne',sans-serif"}}>Templates só para iniciar operação repetida.</div>
            <p style={{fontSize:12,color:C.muted,lineHeight:1.5,margin:"5px 0 0"}}>Documento específico continua em Documentos. Revisão de cliente continua em Video Review.</p>
          </div>
          <Btn onClick={()=>setTab("studio")} variant="ghost" size="sm">Abrir documentos</Btn>
        </div>
      </Card>
    </div>
  );
...
const RevenueOSScore = ({state,setTab,privacyMode,isAdmin,onToggleMoney})=>{
  ...
  const checks=[
    ...
    {label:"Acesso interno definido",done:hasPlanAccess(state,"solo",isAdmin),tab:"business",hint:"Confirme a operação e a liberação de acesso."},
  ];
  const score=Math.round(checks.filter(c=>c.done).length/checks.length*100);
  const pipeline=clients.filter(c=>normalizeClientStatus(c)!=="pago").reduce((a,c)=>a+Number(c.value||0)*(Number(c.closeProbability||c.probability||50)/100),0);
  return (
        ...
        <div style={{textAlign:"center"}}>
          <div style={{width:128,height:128,borderRadius:"50%",margin:"0 auto",display:"grid",placeItems:"center",background:`conic-gradient(${score>=70?"#10b981":score>=40?C.orange:"#eab308"} ${score*3.6}deg, rgba(255,255,255,.07) 0deg)`,boxShadow:"0 24px 70px rgba(0,0,0,.35)"}}>
            <div style={{width:104,height:104,borderRadius:"50%",background:"#151515",display:"grid",placeItems:"center"}}>
              <div><div style={{fontSize:30,fontWeight:900,color:"#fff",fontFamily:"'Syne',sans-serif",lineHeight:1}}>{score}%</div><div style={{fontSize:9,color:C.muted,fontWeight:900,letterSpacing:".12em",textTransform:"uppercase",marginTop:4}}>Does Not Zero</div></div>
            </div>
          </div>
        </div>
        <div>
...
const ExecutiveBriefing = ({state,setTab,privacyMode})=>{
  ...
  const reviewItems=state.reviewDeliverables||[];
  const reviewOpen=reviewItems.filter(r=>!["approved","rejected"].includes(r.status)).length;
  const activeClients=clients.filter(c=>normalizeClientStatus(c)==="em_producao").length;
  const pipelineValue=clients.filter(c=>normalizeClientStatus(c)!=="pago").reduce((a,c)=>a+Number(c.value||0)*(Number(c.closeProbability||50)/100),0);
  ...
  const cards=[
    {label:"Video Review",value:reviewOpen,note:"links de cliente aguardando decisão",tab:"videoReview",color:"#06b6d4"},
...
const TabDashboard = ({state,dispatch,quoteIdx,setTab,privacyMode,setPrivacyMode,userName,isAdmin})=>{
  ...
  const pendingFollowUps=(state.clients||[]).filter(c=>c.followUpDate&&dayDiff(c.followUpDate)<=0&&normalizeClientStatus(c)!=="pago");
  const reviewItems=state.reviewDeliverables||[];
  const reviewsWaiting=reviewItems.filter(r=>r.status==="waiting_review").length;
  const reviewsWithAdjustments=reviewItems.filter(r=>["revision_requested","approved_with_changes"].includes(r.status)).length;
  ...
  const attention=[
    ...
    reviewsWithAdjustments&&{label:`${reviewsWithAdjustments} review${reviewsWithAdjustments>1?"s":""} com ajuste`,tab:"videoReview",color:"#06b6d4"},
    reviewsWaiting&&{label:`${reviewsWaiting} review${reviewsWaiting>1?"s":""} aguardando cliente`,tab:"videoReview",color:"#eab308"},
  ...
  const dailyActions=[
    ...
    reviewsWithAdjustments&&{title:"Resolver ajustes de vídeo",text:`${reviewsWithAdjustments} revisão${reviewsWithAdjustments>1?"ões":"ão"} precisa virar nova versão.`,tab:"videoReview",color:"#06b6d4"},
    reviewsWaiting&&{title:"Acompanhar Video Review",text:`${reviewsWaiting} link${reviewsWaiting>1?"s":""} aguardando aprovação do cliente.`,tab:"videoReview",color:"#eab308"},
  ...
  const primaryAction=dailyActions[0]||{title:"Comece por um cliente",text:"Cadastre ou atualize um cliente para a DNZ Central montar o resto da operação.",tab:"clients",color:C.orange};
  ...
  const quickActions=[
    ...
    {title:"Nova proposta",text:"PDF comercial DNZ pronto para enviar.",tab:"proposta",color:C.orange},
    ...
    <div className="page-stack">
        ...
        <div className="elite-dashboard-grid">
          <div>
            ...
            <div style={{marginTop:16,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
              <Tag color={C.orange}>DNZ Films</Tag>
          ...
          <div className="elite-command-panel">
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              <button className="elite-secondary" onClick={()=>setTab("videoReview")} style={{minHeight:38,padding:"0 10px"}}>Review</button>
              <button className="elite-secondary" onClick={()=>setTab("clients")} style={{minHeight:38,padding:"0 10px"}}>Clientes</button>
            </div>
          </div>
        </div>
      </Card>
      <div className="today-command-grid">
        <div className="dashboard-main">
          <Card className="today-priority-card">
            <div className="page-hero-row">
              <div>
                <div style={{fontSize:11,color:primaryAction.color,fontWeight:900,letterSpacing:".14em",textTransform:"uppercase",marginBottom:7}}>Próxima ação</div>
                <div style={{fontSize:24,color:"#fff",fontFamily:"'Syne',sans-serif",fontWeight:900,lineHeight:1.08}}>{primaryAction.title}</div>
                <p style={{fontSize:13,color:"#aaa",lineHeight:1.55,margin:"8px 0 0",maxWidth:640}}>{primaryAction.text}</p>
              </div>
            ...
            <div className="today-kpi-grid" style={{marginTop:18}}>
              {[
                {label:"Clientes para responder",value:pendingFollowUps.length,color:"#ff2400",tab:"clients"},
                {label:"Reviews aguardando",value:reviewsWaiting+reviewsWithAdjustments,color:"#06b6d4",tab:"videoReview"},
                {label:"Projetos em produção",value:pendingVideos,color:"#8b5cf6",tab:"projects"},
          ...
          <Card className="today-panel">
            <SectionTitle>Fila do dia</SectionTitle>
            {attention.length===0&&<div style={{fontSize:13,color:C.muted}}>Sem alertas críticos agora. O sistema está limpo para execução.</div>}
            {attention.length>0&&<div className="today-action-list">
              {attention.slice(0,7).map((a,i)=><button key={i} onClick={()=>setTab(a.tab)} className="today-action-row" style={{"--row-color":a.color}}>
                <span>{a.label}</span>
                <b>Abrir</b>
              </button>)}
            </div>}
        ...
        <aside className="dashboard-rail">
          ...
          <Card className="today-panel">
            <SectionTitle>Agenda próxima</SectionTitle>
            {upcomingMeetings.length===0&&todayProjectSteps.length===0&&<div style={{fontSize:12,color:C.muted,lineHeight:1.5}}>Nenhuma reunião ou marco de produção para hoje. Bom momento para avançar Video Review ou proposta.</div>}
            {upcomingMeetings.slice(0,4).map(c=>(
              <button key={c.id} onClick={()=>setTab("clients")} className="today-mini-row">
                <span className="private-data">{c.name}</span>
                <b>{new Date(c.nextMeeting+"T00:00").toLocaleDateString("pt-BR",{day:"2-digit",month:"short"})}</b>
              </button>
            ))}
            {todayProjectSteps.slice(0,3).map((p,i)=>(
              <button key={`${p.video.id||p.video.title}-${i}`} onClick={()=>setTab("projects")} className="today-mini-row">
                <span>{p.step.label}</span>
                <b>Hoje</b>
              </button>
            ))}
          </Card>
          {!state.business?.profile&&<Card>
            ...
            <p style={{fontSize:12,color:C.muted,lineHeight:1.5,margin:"-4px 0 12px"}}>Escolha uma base e a DNZ Central adapta serviços e ticket.</p>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
        ...
        <button onClick={()=>setShowDashboardDetails(v=>!v)} className="elite-secondary" style={{minHeight:38,padding:"0 14px"}}>
          {showDashboardDetails?"Ocultar fila operacional":"Ver fila operacional"}
	        </button>
	      </div>
	      ...
	      {showDashboardDetails&&<div className="summary-strip">
	        {[
          {v:reviewsWaiting,l:"Reviews aguardando",c:"#06b6d4",icon:"◉"},
          {v:pendingTasks.length,l:"Atividades abertas",c:"#fb923c",icon:"✓"},
          {v:pendingVideos,l:"Projetos em produção",c:"#8b5cf6",icon:"▶"},
          {v:(state.clients||[]).filter(c=>normalizeClientStatus(c)==="em_producao").length,l:"Em produção",c:"#8b5cf6",icon:"◈"},
        ].map((s,i)=>(
        ...
        <aside className="side-panel">
          <Card style={{padding:"16px 18px"}}>
            ...
            {(state.reviewDeliverables||[]).slice(0,4).map(r=>(
              <button key={r.id||r.review_token} onClick={()=>setTab("videoReview")} style={{width:"100%",display:"flex",justifyContent:"space-between",gap:10,alignItems:"center",marginBottom:8,padding:"10px 11px",borderRadius:12,border:`1px solid ${C.border}`,background:"rgba(255,255,255,.035)",color:"#ddd",fontFamily:"inherit",cursor:"pointer",textAlign:"left"}}>
                <span style={{fontSize:12,fontWeight:900}}>{r.title}</span>
                <Tag color={r.status==="approved"?"#10b981":r.status==="waiting_review"?"#eab308":C.orange}>{r.status==="approved"?"Aprovado":r.status==="waiting_review"?"Aguardando":"Ajustes"}</Tag>
              </button>
            ))}
            {(state.reviewDeliverables||[]).length===0&&<div style={{fontSize:12,color:C.muted,lineHeight:1.5}}>Nenhum review publicado ainda.</div>}
          </Card>
        </aside>
	      </div>}
	    </div>
...
const NAV_GROUPS=[
  ...
  {label:"Operação",items:["dashboard","tasks","agenda","projects"]},
  {label:"Comercial",items:["clients","proposta","finance"]},
  {label:"Produção",items:["studio","templates"]},
  {label:"Sistema",items:["business","export","about"]},
];
const BEGINNER_TABS = ["dashboard","tasks","agenda","projects","clients","proposta","finance","studio","videoReview","business","export"];
...
function App({initialSession=null}={}){
  ...
  const [salesDismissed,setSalesDismissed]=useState(true);
  ...
  const business=normalizeBusiness(state.business);
  const businessOnboardingKey=session?.user?.id?`dnz_business_onboarding_${session.user.id}`:"dnz_business_onboarding_guest";
  const userName=getUserName(session);
  const isAdmin=isAdminSession(session);
  const publicReviewToken=routePath.startsWith("/review/")?decodeURIComponent(routePath.split("/review/")[1]||""):new URLSearchParams(window.location.search).get("review")||"";
  const hasFullAccess=isAdmin;
  const publicTabs=[];
  const canUseWorkspace=isAdmin;
  ...
  useEffect(()=>{
    if(routePath==="/login"&&isAdmin)navigateTo("/app");
  },[routePath,isAdmin,navigateTo]);
  useEffect(()=>{
  ...
  useEffect(()=>{
    if(!session?.user||!isAdmin||!supabaseRef.current)return;
    let active=true;
    (async()=>{
      try{
  ...
  useEffect(()=>{
    if(!isAdmin||!session?.user||business.onboarded)return;
    if(localStorage.getItem(businessOnboardingKey)==="done")return;
    setBusinessOnboardingOpen(true);
  },[isAdmin,session?.user?.id,business.onboarded,businessOnboardingKey]);
  useEffect(()=>{
  ...
  const goTab=id=>{
    ...
    if(!canUseWorkspace&&!publicTabs.includes(id)){
      navigateTo("/login");
      setNavOpen(false);
      notify("Acesso restrito ao admin DNZ.","info",false);
      return;
    }
  ...
  const signInGitHub=async()=>{
    ...
    try{
      setCloudStatus("loading");
      const base=window.location.origin==="null"?window.location.href.split("#")[0]:`${window.location.origin}/app`;
      const {error}=await supabaseRef.current.auth.signInWithOAuth({provider:"github",options:{redirectTo:base}});
  ...
  const lazyTabShared=useMemo(()=>({
    ...
    STATUS_COLORS,VIDEO_COLORS,BADGES,MONTHS,PAG_COLORS,CLIENT_PIPELINE,normalizeClientStatus,clientStatusLabel
  }),[]);
  const Brand=()=>(
    <div style={{display:"flex",alignItems:"center",gap:10}}>
  ...
  if(routePath!=="/app"){
    if(routePath==="/login"){
      return <LoginPage session={session} isAdmin={isAdmin} adminEmails={ADMIN_EMAILS} cloudStatus={cloudStatus} onLogin={signInGitHub} onLogout={signOut} onHome={()=>navigateTo("/")}/>;
    }
    ...
    <div className={`app-shell ${!canUseWorkspace?"public-shell":""} ${compactMode?"compact":""} ${privacyMode?"privacy":""} ${sidebarCollapsed?"sidebar-collapsed":""}`}>
      ...
      <aside className="desktop-sidebar">
        ...
        {canUseWorkspace&&<div style={{marginTop:"auto",paddingTop:12,display:"flex",gap:6,alignItems:"center",justifyContent:"space-between"}}>
          <button onClick={()=>goTab("proposta")} title="Nova proposta" className="utility-btn" style={{flex:1,height:34,fontSize:10,color:C.orange}}>Proposta</button>
      ...
      <BusinessOnboarding open={businessOnboardingOpen} business={business} dispatch={dispatch} onClose={()=>{localStorage.setItem(businessOnboardingKey,"done");if(!business.onboarded)dispatch({type:"UPDATE_BUSINESS",data:{onboarded:true},silent:true});setBusinessOnboardingOpen(false);}}/>
      ...
      <main id="main-content" className="app-content" tabIndex="-1">
        <div className="content-inner">
          ...
          {tab==="about"     &&<TabAbout      session={session} onEnter={()=>session?goTab("dashboard"):signInGitHub()} setTab={goTab}/>}
